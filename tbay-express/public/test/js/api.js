  ...
  function testAPIs() {
    ...
    callAPI('GET', '/api/list', null, null)
      .then((list) => {
        ...
        data.append('listPrice', 100)
        data.append('image', input.files[0])
        callAPI('POST', '/api/post', null, data).then((entry) => {
          entryId = entry._id
          savedEntry = entry // keep a handle to the created photo object
          console.log('\n\n***************************\ncreate results:')
          console.log(entry)
          // find
          callAPI('GET', '/api/find/' + entryId, null, null).then((entry) => {
            console.log('\n\n***************************\nfind results:')
            console.log(entry)

            // update
            testJSON.description += ' appended by the AJAX API '
            callAPI('PUT', '/api/put/' + entryId, null, savedEntry).then(
              (entry) => {
                console.log('\n\n***************************\nupdate results:')
                console.log(entry)

                //delete
                callAPI('DELETE', '/api/delete/' + entryId, null, null).then(
                  (result) => {
                    console.log(
                      '\n\n***************************\ndelete result:'
                    )
